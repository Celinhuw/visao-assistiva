...
import { setServerSession, SessionExpirationSeconds, } from "../../helpers/getSetServerSession.js";
...
export async function handle(request) {
    try {
        ...
        const now = new Date();
        const windowStart = new Date(now.getTime() - RATE_LIMIT_CONFIG.lockoutWindowMinutes * 60 * 1000);
        ...
        const result = await db.transaction().execute(async (trx) => {
            ...
            if (rateLimitQuery &&
                ...
                safeLastFailedAt) {
                const lockoutEnd = new Date(safeLastFailedAt.getTime() +
        ...
        if (Math.random() < RATE_LIMIT_CONFIG.cleanupProbability) {
            const cleanupBefore = new Date(now.getTime() - RATE_LIMIT_CONFIG.lockoutWindowMinutes * 60 * 1000);
            try {
