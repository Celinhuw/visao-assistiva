...
const VisionSystemContext = createContext(undefined);
export const VisionSystemProvider = ({ children }) => {
    ...
    const speak = useCallback((text) => {
        ...
        let rate = 1.0;
        if (settings?.speechRate === "slow")
            rate = 0.8;
        if (settings?.speechRate === "fast")
            rate = 1.2;
        utterance.rate = rate;
    ...
    const startVision = useCallback(async (selectedMode) => {
        try {
            setError(null);
            if (synthesisRef.current)
                synthesisRef.current.cancel();
    ...
    return (_jsx(VisionSystemContext.Provider, { value: {
            isActive,
            mode,
            currentDescription,
            isProcessing,
...
export const useVisionSystem = () => {
    ...
    if (context === undefined) {
        throw new Error("useVisionSystem deve ser usado dentro de um VisionSystemProvider");
    }
    return context;
};
