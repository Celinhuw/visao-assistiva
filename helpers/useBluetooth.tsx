...
export type VisionModeType = 'full' | 'smart' | 'off' | null;
...
export const BluetoothProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [visionMode, setVisionMode] = useState<VisionModeType>(null);
  const [error, setError] = useState<string | null>(null);
  const [isSupported, setIsSupported] = useState<boolean>(false);

  const parseVisionMode = (val: number): VisionModeType => {
    if (val === 0x01) return 'smart';
    if (val === 0x02) return 'full';
    return 'off';
  };
  ...
  const connect = useCallback(async (id: string, name?: string) => {
    try {
      ...
      else setDeviceName("Óculos Vision");
